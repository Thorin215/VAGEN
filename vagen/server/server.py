...
class BatchEnvServer:
    ...
    def _setup_routes(self):
        ...
        def create_environments_batch():
            ...
            # Basic payload validation: each config should contain env_name and env_config
            try:
                if not isinstance(ids2configs, dict) or not ids2configs:
                    raise ValueError("ids2configs must be a non-empty dict of {env_id: config}")
                for env_id, cfg in ids2configs.items():
                    if not isinstance(cfg, dict):
                        raise ValueError(f"Config for environment {env_id} must be a dict")
                    if 'env_name' not in cfg:
                        raise ValueError(f"Config for environment {env_id} is missing 'env_name'")
                    if 'env_config' not in cfg:
                        raise ValueError(f"Config for environment {env_id} is missing 'env_config'")
                ...
                # Unexpected internal error — include traceback in server logs
                import traceback
                traceback.print_exc()
