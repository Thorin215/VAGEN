...
class BatchEnvClient:
    ...
    def _make_request(self, endpoint: str, method: str = "POST", data: Any = None) -> Any:
        ...
        try:
            ...
            if not response.ok:
                # Try to include server-provided error details in the exception
                err_detail = None
                try:
                    payload = response.json()
                    if isinstance(payload, dict):
                        err_detail = payload.get("error") or payload
                    else:
                        err_detail = payload
                except Exception:
                    err_detail = response.text
                raise requests.HTTPError(
                    f"{response.status_code} {response.reason}: {err_detail}",
                    response=response,
                )
